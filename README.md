☕ 歐客佬翡翠咖啡 - 旗艦訂購系統
這是一個為歐客佬翡翠咖啡打造的輕量化前端訂購網頁。系統整合了商品展示、自動化優惠計算、購物車管理，並透過 Google Apps Script 將訂單資訊即時同步至 Google Sheet 試算表。
...
└── images/         # 商品圖片資料夾
🚀 快速上手
下載專案：將所有檔案下載並放置於同一資料夾。

配置圖片：確保 images/ 資料夾中包含 script.js 內所引用的圖片檔名。
